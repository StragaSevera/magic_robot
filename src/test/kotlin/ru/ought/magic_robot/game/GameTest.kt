...
class GameTest : FunSpec({
    test("it works") {
        ...
        val input = slot<Char>()
        val dummy = DummyScreen(mockk())
        every { screen1.process(capture(input)) } returns ScreenMessage(GameMessage.render("test"), screen2)
        every { screen2.process(capture(input)) } returns ScreenMessage(GameMessage.close(), dummy)
        ...
        expect { game.process('c') }.toThrow<IllegalStateException>()
    }
})